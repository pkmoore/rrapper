...
def main():
  ...
  with open('/proc/sys/kernel/randomize_va_space', 'r') as proc:
    if proc.read() != '0':
      print('ASLR should be disabled. Set /proc/sys/kernel/randomize_va_space = 0')
      sys.exit(1)
  ...
  with open('/proc/sys/kernel/yama/ptrace_scope', 'r') as proc:
    if proc.read() != '0':
      print('ptrace on processes should be allowed. Set /proc/sys/kernel/yama/ptrace_scope = 0')
  ...
  try:
    with open(os.devnull, 'w') as fnull:
      subprocess.check_call(['rr', 'help'], stdout=fnull, stderr=subprocess.STDOUT)
