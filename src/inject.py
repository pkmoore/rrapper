...
from syscall_handler_tables import arch_get_handlers, arch_get_debug_printers, arch_get_forgers
...
def debug_handle_syscall(pid, syscall_id, syscall_object, entering):
  ...
  try:
    ...
    try:
      arch_get_debug_printers()[syscall_id](pid, syscall_id, syscall_object)
    except KeyError:
      logger.warning('No debug printer available for syscall {}'
                     .format(syscall_id))
    raise
